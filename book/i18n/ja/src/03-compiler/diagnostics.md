...
診断はデバッグダンプではなく、言語体験の一部です。字句と構文のファミリー (`NDO1xxx`) は実装済みです。ファイルは安定したコード、正確なスパン、expected/found の対とともに拒否されます。型、エフェクト、ケイパビリティのファミリーは計画中であり、それらが示さなければならない連鎖は `spec/errors.md` に仕様化されています。
